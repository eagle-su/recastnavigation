...
static void insertSort(AreaType* a, const int n)
{
	int i, j;
	for (i = 1; i < n; i++)
	{
		const AreaType value = a[i];
...
bool rcMedianFilterWalkableArea(rcContext* ctx, rcCompactHeightfield& chf)
{
	...
	AreaType* areas = (AreaType*)rcAlloc(sizeof(AreaType)*chf.spanCount, RC_ALLOC_TEMP);
	if (!areas)
	{
	...
	// Init distance.
	memset(areas, 0xff, sizeof(AreaType)*chf.spanCount);
	...
	memcpy(chf.areas, areas, sizeof(AreaType)*chf.spanCount);
